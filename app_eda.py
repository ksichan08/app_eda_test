...
class EDA:
    def __init__(self):
        ...
        with tabs[1]:
            ...
            future_year = 2035
            future_pop = nat['총인구수'].iloc[-1] + net_change * (future_year - nat['연도'].max())

            plot_df = nat[['연도', '총인구수']].copy()
            plot_df.loc[len(plot_df)] = [future_year, future_pop]

            fig, ax = plt.subplots()
            sns.lineplot(x='연도', y='총인구수', data=plot_df, marker='o', ax=ax)
        ...
        with tabs[2]:
            ...
            latest = latest[latest['지역'] != '전국'].sort_values(by='총인구수', ascending=False)
            fig, ax = plt.subplots(figsize=(10, 6))
            sns.barplot(x='총인구수', y='지역(영문)', data=latest, ax=ax)
        ...
        with tabs[3]:
            ...
            top100['총인구수'] = top100['총인구수'].apply(lambda x: f'{int(x):,}')
            top100['증감'] = top100['증감'].astype(int)

            styled = top100[['연도', '지역(영문)', '총인구수', '증감']].style.applymap(color_diff, subset=['증감']).format({'증감': '{:,}'})
            st.dataframe(styled, use_container_width=True)

        # 시각화 - 누적 영역 그래프
        with tabs[4]:
            st.subheader("📊 Stacked Area Chart")
            area_df = df[df['지역'] != '전국'].pivot_table(index='연도', columns='지역(영문)', values='총인구수')
